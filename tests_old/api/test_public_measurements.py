...
from iris.commons.models.diamond_miner import FlowMapper, Tool, ToolParameters
...
def measurement1(user):
    return Measurement(
        uuid=uuid.uuid4(),
        user_id=user.id,
        state=MeasurementAgentState.Unknown,
...
def test_get_measurements(api_client_sync, monkeypatch):
    measurements_ = [
        Measurement(
            uuid=uuid.uuid4(),
            user_id=uuid.uuid4(),
            state=MeasurementAgentState.Finished,
            tool=Tool.DiamondMiner,
            agents=[],
            tags=[],
            start_time=datetime.utcnow(),
    ...
    override(
        api_client_sync,
        get_redis,
        fake_redis_factory(measurement_state=MeasurementAgentState.Finished),
    )
...
def test_get_measurement_by_uuid_waiting(
    ...
    override(
        api_client_sync,
        get_redis,
        fake_redis_factory(measurement_state=MeasurementAgentState.Waiting),
    )
    override(api_client_sync, get_storage, fake_storage_factory([target23]))
    monkeypatch.setattr(agents, "all", async_mock([measurement_agent1]))
    monkeypatch.setattr(measurements, "get", async_mock(measurement1))
    expected = measurement1.copy(
        update={
            "state": MeasurementAgentState.Waiting,
            "agents": [
                measurement_agent1.copy(update={"state": MeasurementAgentState.Waiting})
            ],
        }
    )
    response = api_client_sync.get(f"/measurements/public/{measurement1.uuid}")
